...
class Controll {
    ...
    protected function setTela($arquivo, array $diretorio = null) {
        ...
        $view = (strpos($arquivo, '.') === false) ? $arquivo . self::EXTENSAO_DEFAULT : $arquivo;
        ...
        if ((defined('self::TELA_DEFAULT')) && (trim(self::TELA_DEFAULT) != '')) {
            ...
            include_once(VIEW . DS . self::TELA_DIRECTORY_DEFAULT . DS . ((strpos(self::TELA_DEFAULT, '.') === false) ? self::TELA_DEFAULT . self::EXTENSAO_DEFAULT : self::TELA_DEFAULT));
...
//			$_SESSION['page'] = (!empty($_GET['url'])) ? $_GET['url'] : '';
        ...
        if (!empty($_GET))
            foreach ($_GET as $indice => $valor) {
                $string .= ( $indice != 'url') ? "?" . $indice . "=" . $valor : $valor;
            }
